...
> 长期价值投资辅助系统：基于确定性规则 + LLM 增强的 MAG7 股票分析工具

## 🎯 项目概览

DeepValue 是一个自动化的股票分析系统，专注于 MAG7（AAPL, MSFT, AMZN, NVDA, META, TSLA, GOOGL, GOOG）+ QQQ 的长期价值评估。系统通过确定性算法计算 DeepValue Score、支撑/阻力位，并结合外部信号源（Perplexity）和 LLM 策略增强，生成每日中文分析报告。
...
- 强制 `reserve_pct ≥ 25%`
- `accumulate` 需 `score ≥ ACCUMULATE_SCORE_MIN`
- 权重必须单调：S1 ≤ S2 ≤ S3
...
本系统仅供学习和研究使用，不构成任何投资建议。使用者需自行评估风险，并对投资决策负责。
